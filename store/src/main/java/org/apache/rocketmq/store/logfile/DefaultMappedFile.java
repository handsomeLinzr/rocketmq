...
public class DefaultMappedFile extends AbstractMappedFile {
    ...
    private void init(final String fileName, final int fileSize) throws IOException {
        ...
        this.fileFromOffset = Long.parseLong(this.file.getName());  // 偏移量
        boolean ok = false;

        UtilAll.ensureDirOK(this.file.getParent());  // 递归创建文件

        try {
            this.fileChannel = new RandomAccessFile(this.file, "rw").getChannel();
            this.mappedByteBuffer = this.fileChannel.map(MapMode.READ_WRITE, 0, fileSize);  // mmap映射
    ...
    public AppendMessageResult appendMessagesInner(final MessageExt messageExt, final AppendMessageCallback cb,
        ...
        int currentPos = WROTE_POSITION_UPDATER.get(this);  // 获得写位置

        if (currentPos < this.fileSize) {
            ByteBuffer byteBuffer = appendMessageBuffer().slice();  // 获得引用
            byteBuffer.position(currentPos);
            ...
            WROTE_POSITION_UPDATER.addAndGet(this, result.getWroteBytes());  // 更新wrotePos
            this.storeTimestamp = result.getStoreTimestamp();
            return result;
        }
        log.error("MappedFile.appendMessage return null, wrotePosition: {} fileSize: {}", currentPos, this.fileSize);
    ...
    public boolean appendMessage(final byte[] data, final int offset, final int length) {
        ...
        if ((currentPos + length) <= this.fileSize) {
            try {
                ByteBuffer buf = this.mappedByteBuffer.slice();   // 这里是获得buffer的一个切片引用
                buf.position(currentPos);
                buf.put(data, offset, length);
            } catch (Throwable e) {
                log.error("Error occurred when append message to mappedFile.", e);
            }
            WROTE_POSITION_UPDATER.addAndGet(this, length);   // 增加当前可写位置的偏移
            return true;
        }
    ...
    protected boolean isAbleToCommit(final int commitLeastPages) {
        ...
        if (commitLeastPages > 0) {
            return ((write / OS_PAGE_SIZE) - (commit / OS_PAGE_SIZE)) >= commitLeastPages;  // 默认commitLeastPages=4，即默认超过4页未提交，则进行提交
        }
    ...
    @Override
    public SelectMappedBufferResult selectMappedBuffer(int pos) {  // 从map文件获取buffer数据
        int readPosition = getReadPosition();  // 可读的位置
        if (pos < readPosition && pos >= 0) {
            if (this.hold()) {
                this.mappedByteBufferAccessCountSinceLastSwap++;
                ByteBuffer byteBuffer = this.mappedByteBuffer.slice();
                byteBuffer.position(pos);  // 开始读取位置
                int size = readPosition - pos;  // 读取数据大小
                ByteBuffer byteBufferNew = byteBuffer.slice();
                byteBufferNew.limit(size);
                return new SelectMappedBufferResult(this.fileFromOffset + pos, byteBufferNew, size, this);  // this.fileFromOffset + pos = 物理偏移
            }
        }
