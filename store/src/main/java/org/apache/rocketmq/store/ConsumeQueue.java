...
public class ConsumeQueue implements ConsumeQueueInterface, FileQueueLifeCycle {
    ...
    public static final int CQ_STORE_UNIT_SIZE = 20;  // consumeQueue中每个单元的存储大小  8+4+8（commitLog offset=8, size=4, msg tag hashcode=8）
    ...
    public ConsumeQueue(
        ...
        this.byteBufferIndex = ByteBuffer.allocate(CQ_STORE_UNIT_SIZE);  // 构建单条记录的内存空间
    ...
    public void putMessagePositionInfoWrapper(DispatchRequest request) {
        ...
        for (int i = 0; i < maxRetries && canWrite; i++) {
            long tagsCode = request.getTagsCode();  // tagsHashCode
            if (isExtWriteEnable()) {
            ...
            boolean result = this.putMessagePositionInfo(request.getCommitLogOffset(),
                request.getMsgSize(), tagsCode, request.getConsumeQueueOffset());  // 序列化consumeQueue
            if (result) {
                if (this.messageStore.getMessageStoreConfig().getBrokerRole() == BrokerRole.SLAVE ||
                    this.messageStore.getMessageStoreConfig().isEnableDLegerCommitLog()) {
                    this.messageStore.getStoreCheckpoint().setPhysicMsgTimestamp(request.getStoreTimestamp());  // 设置设置时间
                }
                this.messageStore.getStoreCheckpoint().setLogicsMsgTimestamp(request.getStoreTimestamp());  // 设置本机记录时间
    ...
    private boolean putMessagePositionInfo(final long offset, final int size, final long tagsCode,
        ...
        this.byteBufferIndex.limit(CQ_STORE_UNIT_SIZE);  // 20 bytes
        this.byteBufferIndex.putLong(offset);   // commitLog Offset
        this.byteBufferIndex.putInt(size);  // msg size
        this.byteBufferIndex.putLong(tagsCode);  // tagsHashCode

        final long expectLogicOffset = cqOffset * CQ_STORE_UNIT_SIZE;  // cqOffset表示偏移的下标（逻辑偏移），乘 20bytes得到物理偏移
        ...
        if (mappedFile != null) {
            ...
            if (mappedFile.isFirstCreateInQueue() && cqOffset != 0 && mappedFile.getWrotePosition() == 0) {
                ...
                this.fillPreBlank(mappedFile, expectLogicOffset);  // 填充空白数据
            ...
            this.maxPhysicOffset = offset + size;  // 记录commitLog的最大物理偏移
            return mappedFile.appendMessage(this.byteBufferIndex.array());  // map文件映射
        }
        return false;
    }
    ...
    public SelectMappedBufferResult getIndexBuffer(final long startIndex) {  // 根据偏移获得buffer数据
    ...
    private class ConsumeQueueIterator implements ReferredIterator<CqUnit> {
        ...
        private int relativePos = 0;  // 从sbr中的buffer获取的位置
        ...
        public CqUnit next() {
            ...
            long queueOffset = (sbr.getStartOffset() + sbr.getByteBuffer().position() -  relativePos) / CQ_STORE_UNIT_SIZE;  // 数据偏移(数组脚标)
            CqUnit cqUnit = new CqUnit(queueOffset,
                    sbr.getByteBuffer().getLong(),  // commitLog偏移
                    sbr.getByteBuffer().getInt(),   // size大小
                    sbr.getByteBuffer().getLong());   // tagsHashCode
