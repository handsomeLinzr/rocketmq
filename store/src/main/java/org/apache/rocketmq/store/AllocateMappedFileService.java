...
public class AllocateMappedFileService extends ServiceThread {
    ...
    private ConcurrentMap<String, AllocateRequest> requestTable =
        new ConcurrentHashMap<>();   // filePath:请求
    private PriorityBlockingQueue<AllocateRequest> requestQueue =
        new PriorityBlockingQueue<>();   // 请求队列
    ...
    public MappedFile putRequestAndReturnMappedFile(String nextFilePath, String nextNextFilePath, int fileSize) {
        int canSubmitRequests = 2;  // 允许提交请求的次数
        ...
        boolean nextPutOK = this.requestTable.putIfAbsent(nextFilePath, nextReq) == null;  // 避免此时并发有其他也在创建这个文件，需要这一步校验
        ...
        if (nextPutOK) {
            ...
            boolean offerOK = this.requestQueue.offer(nextReq);  // 放入请求到请求队列，后台线程会获取并进行创建mmapfile
            if (!offerOK) {
    ...
    public void run() {
        ...
        while (!this.isStopped() && this.mmapOperation()) {  // 一直在运行
    ...
    private boolean mmapOperation() {
        ...
        try {
            req = this.requestQueue.take();  // 获得请求
            ...
            if (req.getMappedFile() == null) {
                ...
                if (messageStore.isTransientStorePoolEnable()) {
                    ...
                    mappedFile = new DefaultMappedFile(req.getFilePath(), req.getFileSize());  // 创建映射文件
                }
