...
public class IndexService {
    ...
    public IndexService(final DefaultMessageStore store) {
        ...
        this.hashSlotNum = store.getMessageStoreConfig().getMaxHashSlotNum();  // 500w   hashcode槽数量
        this.indexNum = store.getMessageStoreConfig().getMaxIndexNum();  // 500w * 4
        this.storePath =
    ...
    public void buildIndex(DispatchRequest req) {
        IndexFile indexFile = retryGetAndCreateIndexFile();  // 创建index文件
        if (indexFile != null) {
    ...
    public IndexFile retryGetAndCreateIndexFile() {
        ...
        for (int times = 0; null == indexFile && times < MAX_TRY_IDX_CREATE; times++) {
            indexFile = this.getAndCreateLastIndexFile();  // 创建index文件
            if (null != indexFile) {
                break;
            }
    ...
    public IndexFile getAndCreateLastIndexFile() {
...

        {
            this.readWriteLock.readLock().lock();
            if (!this.indexFileList.isEmpty()) {
                IndexFile tmp = this.indexFileList.get(this.indexFileList.size() - 1);  // 获取最后一个文件
                if (!tmp.isWriteFull()) {  // 还能写
                    indexFile = tmp;
                } else {  // 已经满了，则将该文件设置为上一个文件prevIndexFile
        ...
        if (indexFile == null) {  // 创建 index 文件
            ...
            if (indexFile != null) {
                ...
                Thread flushThread = new Thread(new AbstractBrokerRunnable(defaultMessageStore.getBrokerConfig()) {
                    @Override
                    public void run0() {
                        IndexService.this.flush(flushThisFile);  // 强制刷盘
                    }
