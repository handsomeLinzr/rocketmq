...
public class CommitLog implements Swappable {
    ...
    public CommitLog(final DefaultMessageStore messageStore) {
        String storePath = messageStore.getMessageStoreConfig().getStorePathCommitLog();  // 获取commitLog路径
        if (storePath.contains(MixAll.MULTI_PATH_SPLITTER)) {
            ...
            this.mappedFileQueue = new MappedFileQueue(storePath,
                messageStore.getMessageStoreConfig().getMappedFileSizeCommitLog(),  // 默认1G
    ...
    public void start() {
        this.flushManager.start();  // 刷盘管理线程启动
    ...
    public SelectMappedBufferResult getData(final long offset, final boolean returnFirstOnNotFound) {
        ...
        MappedFile mappedFile = this.mappedFileQueue.findMappedFileByOffset(offset, returnFirstOnNotFound);  // 获得对应的文件
        if (mappedFile != null) {
            int pos = (int) (offset % mappedFileSize);  // 获得具体文件内的偏移
    ...
    public DispatchRequest checkMessageAndReturnSize(java.nio.ByteBuffer byteBuffer, final boolean checkCRC,
        ...
        try {
            // 1 TOTAL SIZE
            int totalSize = byteBuffer.getInt();  // 消息大小

            // 2 MAGIC CODE
            int magicCode = byteBuffer.getInt();  // 魔术头
            switch (magicCode) {
            ...
            if (bodyLen > 0) {
                if (readBody) {
                    ...
                    byteBuffer.position(byteBuffer.position() + bodyLen);  // 跳过body
                }
            }
    ...
    public void updateMaxMessageSize(PutMessageThreadLocal putMessageThreadLocal) {
        ...
        int newMaxMessageSize = this.defaultMessageStore.getMessageStoreConfig().getMaxMessageSize();  // 1024 * 1024 * 4
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        msg.setBodyCRC(UtilAll.crc32(msg.getBody()));  // CRC32编码
        // Back to Results
        ...
        String topicQueueKey = generateKey(putMessageThreadLocal.getKeyBuilder(), msg);  // topic-queueId
        long elapsedTimeInLock = 0;
        MappedFile unlockMappedFile = null;
        MappedFile mappedFile = this.mappedFileQueue.getLastMappedFile();  // 获得commitLog文件

        long currOffset;  // 下一个可写的偏移地址
        if (mappedFile == null) {
            currOffset = 0;
        } else {
        ...
        int needAckNums = this.defaultMessageStore.getMessageStoreConfig().getInSyncReplicas();  // 副本ack的数量
        boolean needHandleHA = needHandleHA(msg);  // 判断是否需要ha集群响应

        if (needHandleHA && this.defaultMessageStore.getBrokerConfig().isEnableControllerMode()) {  // todo 集群ha相关判断和处理
        ...
        topicQueueLock.lock(topicQueueKey);  // 锁住topic-queueId
        try {
            ...
            if (needAssignOffset) { // 分配consumeQueue
                defaultMessageStore.assignOffset(msg, getMessageNum(msg));  // 分配consumeQueue
            }

            PutMessageResult encodeResult = putMessageThreadLocal.getEncoder().encode(msg);  // 编码msg
            if (encodeResult != null) {
                return CompletableFuture.completedFuture(encodeResult);
            }
            msg.setEncodedBuff(putMessageThreadLocal.getEncoder().getEncoderBuffer());  // 设置encodedBuff，即msg所有数据编码
            PutMessageContext putMessageContext = new PutMessageContext(topicQueueKey);

            putMessageLock.lock(); //spin or ReentrantLock ,depending on store config    互斥锁或者自旋锁
            try {
                long beginLockTimestamp = this.defaultMessageStore.getSystemClock().now();
                this.beginTimeInLock = beginLockTimestamp;  // 记录加锁时间
                ...
                result = mappedFile.appendMessage(msg, this.appendMessageCallback, putMessageContext);  // !!!保存本地文件!!!
                switch (result.getStatus()) {
                    case PUT_OK:
                        onCommitLogAppend(msg, result, mappedFile);
                        break;
                    case END_OF_FILE:  // 文件内存不够，新增文件重新写msg
                        onCommitLogAppend(msg, result, mappedFile);
    ...
    private CompletableFuture<PutMessageResult> handleDiskFlushAndHA(PutMessageResult putMessageResult,
        ...
        if (!needHandleHA) {
            ...
            replicaResultFuture = handleHA(putMessageResult.getAppendMessageResult(), putMessageResult, needAckNums); // 等待集群响应
        }
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        private final ByteBuffer msgStoreItemMemory;  // 记录上一次空间内存
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // PHY OFFSET
            long wroteOffset = fileFromOffset + byteBuffer.position();  // 物理偏移 = 文件偏移 + 文件内相对偏移
            ...
            Long queueOffset = msgInner.getQueueOffset();   // 消费队列num
            ...
            ByteBuffer preEncodeBuffer = msgInner.getEncodedBuff();  // 获取msg编码后的数据
            final int msgLen = preEncodeBuffer.getInt(0);  // 获取长度

            // Determines whether there is sufficient free space
            if ((msgLen + END_FILE_MIN_BLANK_LENGTH) > maxBlank) {  // 不够空间
                this.msgStoreItemMemory.clear();
                // 1 TOTALSIZE
                this.msgStoreItemMemory.putInt(maxBlank);  // 记录上一次剩下的空间
                // 2 MAGICCODE
                this.msgStoreItemMemory.putInt(CommitLog.BLANK_MAGIC_CODE);  // 标志文件结束
                // 3 The remaining space may be any value
                // Here the length of the specially set maxBlank
                final long beginTimeMills = CommitLog.this.defaultMessageStore.now();
                byteBuffer.put(this.msgStoreItemMemory.array(), 0, 8);  // 添加上边设置的信息，8个字节
            ...
            byteBuffer.put(preEncodeBuffer);   // 写入队列缓冲区
    ...
    class DefaultFlushManager implements FlushManager {
        ...
        @Override public void start() {
            this.flushCommitLogService.start();  // 启动Log刷盘线程
