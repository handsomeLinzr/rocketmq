...
public class MappedFileQueue implements Swappable {
    ...
    public boolean doLoad(List<File> files) {
        ...
        for (File file : files) {
            ...
            try {
                MappedFile mappedFile = new DefaultMappedFile(file.getPath(), mappedFileSize);  // mmap映射文件
    ...
    public MappedFile getLastMappedFile(final long startOffset, boolean needCreate) {
        ...
        if (createOffset != -1 && needCreate) {
            return tryCreateMappedFile(createOffset);  // 创建文件
        }
    ...
    protected MappedFile doCreateMappedFile(String nextFilePath, String nextNextFilePath) {
        ...
        if (mappedFile != null) {
            if (this.mappedFiles.isEmpty()) {
                mappedFile.setFirstCreateInQueue(true);  // 首次创建设置该值为true
            }
            this.mappedFiles.add(mappedFile);
        }
    ...
    public long getMaxOffset() {
        MappedFile mappedFile = getLastMappedFile();   // commitLog中最后一个文件
        if (mappedFile != null) {
    ...
    public MappedFile findMappedFileByOffset(final long offset) {  // 根据偏移获取文件
