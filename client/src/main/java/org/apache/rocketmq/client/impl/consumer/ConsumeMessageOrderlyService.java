...
public class ConsumeMessageOrderlyService implements ConsumeMessageService {
    ...
    class ConsumeRequest implements Runnable {
        ...
        public void run() {
            ...
            final Object objLock = messageQueueLock.fetchLockObject(this.messageQueue);  // queue加锁
            synchronized (objLock) {
                if (MessageModel.BROADCASTING.equals(ConsumeMessageOrderlyService.this.defaultMQPushConsumerImpl.messageModel())
                    || this.processQueue.isLocked() && !this.processQueue.isLockExpired()) {
                    final long beginTime = System.currentTimeMillis();
                    for (boolean continueConsume = true; continueConsume; ) {  // 当 continueConsume=false消费失败退出循环
                        if (this.processQueue.isDropped()) {
                        ...
                        if (!msgs.isEmpty()) {
                            ...
                            try {
                                ...
                                status = messageListener.consumeMessage(Collections.unmodifiableList(msgs), context);  // 消费
                            } catch (Throwable e) {
