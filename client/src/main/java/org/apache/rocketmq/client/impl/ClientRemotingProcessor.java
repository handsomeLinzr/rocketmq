...
public class ClientRemotingProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand checkTransactionState(ChannelHandlerContext ctx,
        ...
        if (messageExt != null) {
            ...
            String transactionId = messageExt.getProperty(MessageConst.PROPERTY_UNIQ_CLIENT_MESSAGE_ID_KEYIDX);  // 事务id
            if (null != transactionId && !"".equals(transactionId)) {
                messageExt.setTransactionId(transactionId);
            }
            final String group = messageExt.getProperty(MessageConst.PROPERTY_PRODUCER_GROUP);  // 生产组
            if (group != null) {
                MQProducerInner producer = this.mqClientFactory.selectProducer(group);  // 获得生产者
                if (producer != null) {
                    final String addr = RemotingHelper.parseChannelRemoteAddr(ctx.channel());
                    producer.checkTransactionState(addr, messageExt, requestHeader);  // 检查
                } else {
