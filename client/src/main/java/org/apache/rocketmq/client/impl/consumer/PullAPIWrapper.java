...
public class PullAPIWrapper {
    ...
    public PullResult processPullResult(final MessageQueue mq, final PullResult pullResult,
        ...
        this.updatePullFromWhichNode(mq, pullResultExt.getSuggestWhichBrokerId());  // 更新获得的节点
        if (PullStatus.FOUND == pullResult.getPullStatus()) {
            ByteBuffer byteBuffer = ByteBuffer.wrap(pullResultExt.getMessageBinary());  // 获得的消息
            List<MessageExt> msgList = MessageDecoder.decodesBatch(   // 解析消息
                byteBuffer,
            ...
            if (!subscriptionData.getTagsSet().isEmpty() && !subscriptionData.isClassFilterMode()) {  // tags过滤
            ...
            for (MessageExt msg : msgListFilterAgain) {
                ...
                if (Boolean.parseBoolean(traFlag)) {  // 事务id
    ...
    public PullResult pullKernelImpl(
        ...
        FindBrokerResult findBrokerResult =   // 获取需要的broker节点信息，优先master，没有则获取slave
            this.mQClientFactory.findBrokerAddressInSubscribe(this.mQClientFactory.getBrokerNameFromMessageQueue(mq),
                this.recalculatePullFromWhichNode(mq), false);
        if (null == findBrokerResult) {
            this.mQClientFactory.updateTopicRouteInfoFromNameServer(mq.getTopic());  // 重新更新
            findBrokerResult =
                this.mQClientFactory.findBrokerAddressInSubscribe(this.mQClientFactory.getBrokerNameFromMessageQueue(mq),
                    this.recalculatePullFromWhichNode(mq), false);
        }
        ...
        if (findBrokerResult != null) {
            ...
            if (findBrokerResult.isSlave()) {  // slave节点，则不进行commit提交操作
