    ...
    public long computePullFromWhereWithException(MessageQueue mq) throws MQClientException {
        ...
        switch (consumeFromWhere) {
            ...
            case CONSUME_FROM_LAST_OFFSET: {
                long lastOffset = offsetStore.readOffset(mq, ReadOffsetType.READ_FROM_STORE);  // 获从broker获取当前MessageQueue消费偏移
                if (lastOffset >= 0) {
                    result = lastOffset;
                }
                // First start,no offset
                else if (-1 == lastOffset) {  // 刚启动无偏移
                    if (mq.getTopic().startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX)) {
                        result = 0L;
                    } else {
                        try {
                            result = this.mQClientFactory.getMQAdminImpl().maxOffset(mq);  // 从broker节点获取commitLog中改topic中最大的偏移
                        } catch (MQClientException e) {
