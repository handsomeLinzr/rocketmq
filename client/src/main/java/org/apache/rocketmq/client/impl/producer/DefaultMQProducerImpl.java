...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    // 指向调用了这个方法创建的producer，在有参构造函数设置
    // 也就是外边创建的 生产者 对象
    private final DefaultMQProducer defaultMQProducer;

    // topic 和对应的 publish 的信息缓存
    ...
    // 有参函数初始化 5000，作为下边 defaultAsyncSenderExecutor 的任务队列
    private final BlockingQueue<Runnable> asyncSenderThreadPoolQueue;

    // 异步发送线程池，有参构造函数处理，构造函数创建
    private final ExecutorService defaultAsyncSenderExecutor;

    protected BlockingQueue<Runnable> checkRequestQueue;

    protected ExecutorService checkExecutor;  // 事务检查线程

    // 状态 调用 start 后转为 RUNNING
    private ServiceState serviceState = ServiceState.CREATE_JUST;

    // start 方法创建被设置了，得到 MQClientInstance
    ...
    // 有参构造函数创建  // 异步限流，默认 1w，公平锁
    private Semaphore semaphoreAsyncSendNum;

    // 有参构造函数创建    // 限流异步发送数量，1024 * 1024
    ...
    public DefaultMQProducerImpl(final DefaultMQProducer defaultMQProducer, RPCHook rpcHook) {
        // 指向调用了这个方法创建的producer
        this.defaultMQProducer = defaultMQProducer;

        this.rpcHook = rpcHook;  // rpc钩子函数，默认null

        this.asyncSenderThreadPoolQueue = new LinkedBlockingQueue<>(50000);  // 队列

        this.defaultAsyncSenderExecutor = new ThreadPoolExecutor(  // 线程池
                // 核心线程数
        ...
        if (defaultMQProducer.getBackPressureForAsyncSendNum() > 10) {  // 设置限流，最小10
            // 信号量，限流  限制 1w
    ...
    public void initTransactionEnv() {
        ...
        if (producer.getExecutorService() != null) {   // 设置checkExecutor线程池，有设置则用自定义的，没有则默认
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            // 启动逻辑，刚 start 后调用到这里
            case CREATE_JUST:
                // 初始化状态，先设置成 START_FAILED
                this.serviceState = ServiceState.START_FAILED;

                // 检查配置，生产者名称不能用默认 DEFAULT_PRODUCER
                this.checkConfig();

                // 判断生产者的组名不为 CLIENT_INNER_PRODUCER，默认自定义的都不是
                if (!this.defaultMQProducer.getProducerGroup().equals(MixAll.CLIENT_INNER_PRODUCER_GROUP)) {
                    // 设置实例名
                    this.defaultMQProducer.changeInstanceNameToPID();
                }

                // 创建 mQClientFactory，得到 MQClientInstance
                this.mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(this.defaultMQProducer, rpcHook); // 实例化mqClient
                ...
                if (startFactory) {
                    // 默认 true
                    // 调用 mQClientFactory start，重点
                    mQClientFactory.start();
                }

                log.info("the producer [{}] start OK. sendMessageWithVIPChannel={}", this.defaultMQProducer.getProducerGroup(),
                    this.defaultMQProducer.isSendMessageWithVIPChannel());
                // 修改 serviceState 服务状态为 RUNNING
    ...
    public void checkTransactionState(final String addr, final MessageExt msg,
        ...
        Runnable request = new Runnable() {
            ...
            public void run() {
                ...
                TransactionListener transactionListener = getCheckListener();  // 生产者自己写的check方法
    ...
    private SendResult sendDefaultImpl(
        ...
        this.makeSureStateOK();  // 状态校验 RUNNING
        Validators.checkMessage(msg, this.defaultMQProducer);  // 校验body
        final long invokeID = random.nextLong();  // 随机id
        long beginTimestampFirst = System.currentTimeMillis();
        long beginTimestampPrev = beginTimestampFirst;
        long endTimestamp = beginTimestampFirst;
        TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic());  // 获取对应的topic队列信息
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            for (; times < timesTotal; times++) {  // 同步最大重试3次
                String lastBrokerName = null == mq ? null : mq.getBrokerName();
                MessageQueue mqSelected = this.selectOneMessageQueue(topicPublishInfo, lastBrokerName); // 获得发送的队列
                if (mqSelected != null) {
                    mq = mqSelected;
                    brokersSent[times] = mq.getBrokerName();  // 记录节点名称
                    try {
                        beginTimestampPrev = System.currentTimeMillis();
                        if (times > 0) {
                            //Reset topic with namespace during resend.
                            msg.setTopic(this.defaultMQProducer.withNamespace(msg.getTopic()));  // 重新设置topic
                        }
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            ...
            this.mQClientFactory.updateTopicRouteInfoFromNameServer(topic);  // 更新topic相关的信息到本地的map中
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (brokerAddr != null) {
            brokerAddr = MixAll.brokerVIPChannel(this.defaultMQProducer.isSendMessageWithVIPChannel(), brokerAddr);  // 节点地址

            byte[] prevBody = msg.getBody();  // 消息体
            try {
                //for MessageBatch,ID has been set in the generating process
                if (!(msg instanceof MessageBatch)) {  // 非批量发送
                ...
                if (this.tryToCompressMessage(msg)) {  // 超过 4K 压缩
                ...
                if (Boolean.parseBoolean(tranMsg)) {  // 事务消息
                ...
                if (requestHeader.getTopic().startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX)) {  // 重试队列
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        throws MQClientException {
        TransactionListener transactionListener = getCheckListener();  // 自定义checkListener
        ...
        SendResult sendResult = null;
        MessageAccessor.putProperty(msg, MessageConst.PROPERTY_TRANSACTION_PREPARED, "true");  // 设置标志
        MessageAccessor.putProperty(msg, MessageConst.PROPERTY_PRODUCER_GROUP, this.defaultMQProducer.getProducerGroup());  // 生产组
        try {
            ...
            case SEND_OK: {
                try {
                    if (sendResult.getTransactionId() != null) {
                        msg.putUserProperty("__transactionId__", sendResult.getTransactionId());  // 设置事务id
                    }
                    ...
                    if (null != localTransactionExecuter) {
                        ...
                        localTransactionState = transactionListener.executeLocalTransaction(msg, arg);  // 调用client编写的executeLocalTransaction获取状态
                    }
                    if (null == localTransactionState) {  // 不回默认 UNKNOW
        ...
        TransactionSendResult transactionSendResult = new TransactionSendResult();  // 构建结果
     ...
     * DEFAULT SYNC -------------------------------------------------------
     */
    public SendResult send(  // 默认同步
        Message msg) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
        return send(msg, this.defaultMQProducer.getSendMsgTimeout());
    }

    public void endTransaction(  // 回给broker事务状态
        final Message msg,
        ...
        String transactionId = sendResult.getTransactionId();  // 事务id
