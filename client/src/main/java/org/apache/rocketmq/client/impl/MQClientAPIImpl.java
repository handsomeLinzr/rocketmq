...
public class MQClientAPIImpl implements NameServerUpdateCallback {
    ...
    // 构建 NettyRemotingClient
    private final RemotingClient remotingClient;

    // 构造函数创建 DefaultTopAddressing
    private final TopAddressing topAddressing;

    // 有参构造时候创建
    // ClientRemotingProcessor，内部封装了 MQClientInstance
    ...
    public MQClientAPIImpl(final NettyClientConfig nettyClientConfig,
        ...
        // 暂时不知道是什么，对系统变量进行获取
        // MixAll.getWSAddr() 获取 ws 的地址，暂时不用，默认是 http://jmenv.tbsite.net/rocketmq/nsaddr
        // clientConfig.getUnitName() 默认 空
    ...
    protected SendResult processSendResponse(
        ...
        SendMessageResponseHeader responseHeader =     // 处理结果
    ...
    private PullResult processPullResponse(
        ...
        PullMessageResponseHeader responseHeader =  // 构建responseHeader
    ...
    public List<String> getConsumerIdListByGroup(
        ...
        RemotingCommand response = this.remotingClient.invokeSync(MixAll.brokerVIPChannel(this.clientConfig.isVipChannelEnabled(), addr),  // 从broker节点获取消费者信息
            request, timeoutMillis);
        assert response != null;
        switch (response.getCode()) {
            case ResponseCode.SUCCESS: {
    ...
    public TopicRouteData getTopicRouteInfoFromNameServer(final String topic, final long timeoutMillis,
        ...
        RemotingCommand response = this.remotingClient.invokeSync(null, request, timeoutMillis);  // 发送请求
        assert response != null;
        switch (response.getCode()) {
