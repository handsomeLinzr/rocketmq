...
public class ConsumeMessageConcurrentlyService implements ConsumeMessageService {
    ...
    public void start() {
        this.cleanExpireMsgExecutors.scheduleAtFixedRate(new Runnable() {  // 单线程线程池
    ...
    private void cleanExpireMsg() {
        ...
        while (it.hasNext()) {
            ...
            pq.cleanExpiredMsg(this.defaultMQPushConsumer);  // 清除超时的消息
        }
    }
        ...
        switch (this.defaultMQPushConsumer.getMessageModel()) {
            ...
            case CLUSTERING:
                ...
                for (int i = ackIndex + 1; i < consumeRequest.getMsgs().size(); i++) {
                    ...
                    boolean result = this.sendMessageBack(msg, context);  // 重新发到broker，等待下次消费
                    if (!result) {
    ...
    public boolean sendMessageBack(final MessageExt msg, final ConsumeConcurrentlyContext context) {
        int delayLevel = context.getDelayLevelWhenNextConsume();  // 重试级别
    ...
    private void submitConsumeRequestLater(
        ...
        this.scheduledExecutorService.schedule(new Runnable() {  // 单线程线程池
    ...
    class ConsumeRequest implements Runnable {
        ...
        public void run() {
            ...
            MessageListenerConcurrently listener = ConsumeMessageConcurrentlyService.this.messageListener;  // 自己注册的消费逻辑
            ConsumeConcurrentlyContext context = new ConsumeConcurrentlyContext(messageQueue);
            ConsumeConcurrentlyStatus status = null;
            defaultMQPushConsumerImpl.tryResetPopRetryTopic(msgs, consumerGroup);  // 获取真实的topic（比如重试队列则去掉retry）
            ...
            try {
                ...
                status = listener.consumeMessage(Collections.unmodifiableList(msgs), context);  // 具体消费
            } catch (Throwable e) {
            ...
            if (ConsumeMessageConcurrentlyService.this.defaultMQPushConsumerImpl.hasHook()) {   // ConsumeMessageTraceHookImpl
