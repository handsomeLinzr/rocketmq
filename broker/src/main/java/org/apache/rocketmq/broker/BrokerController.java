...
public class BrokerController {
    ...
    protected TransactionalMessageService transactionalMessageService;   // 事务信息
    protected AbstractTransactionalMessageCheckListener transactionalMessageCheckListener;  // 检查事务
    ...
    protected void initializeRemotingServer() throws CloneNotSupportedException {
        this.remotingServer = new NettyRemotingServer(this.nettyServerConfig, this.clientHousekeepingService);  // nettyServer
    ...
    protected void initializeBrokerScheduledTasks() {
        ...
        }, initialDelay, period, TimeUnit.MILLISECONDS);  // brokerRecord
        // consumeOffset 消费的记录
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
            @Override
            public void run() {
                try {
                    BrokerController.this.consumerOffsetManager.persist();  // consumeOffset 消费的记录
                } catch (Throwable e) {
                    LOG.error(
        ...
        // filterManager / OrderInfoManager
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
            @Override
            public void run() {
                try {
    ...
    protected void initializeScheduledTasks() {
        ...
        } else if (this.brokerConfig.isFetchNamesrvAddrByAddressServer()) {  // 获取nameSrv的定时任务
            this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
    ...
    public boolean initialize() throws CloneNotSupportedException {
        ...
        boolean result = this.topicConfigManager.load();  // 从 topic.json 序列化topic信息到内存 topicConfigManage.topicConfigTable
        result = result && this.topicQueueMappingManager.load();  // topicQueueMappingTable
        result = result && this.consumerOffsetManager.load();  // offsetTable
        result = result && this.subscriptionGroupManager.load();  //  subscriptionGroupTable
        result = result && this.consumerFilterManager.load();  // filterDataByTopic
        result = result && this.consumerOrderInfoManager.load();  // ConsumerOrderInfoManager.table

        if (result) {
            try {  // 创建DefaultMessageStore的时候会添加一系列对象到dispatcherList中，进行定时分发请求
        ...
        if (messageStore != null) {
            registerMessageStoreHook();  // 注册commitLog存储数据的钩子函数，其中包括重试队列队列-> 延迟队列的钩子逻辑
            result = result && this.messageStore.load();    // 初始化存储文件到内存中，包括 commitLog/consumeQueue/index 等
        }
        ...
        result = result && this.scheduleMessageService.load();   // 只要是初始化延时队列信息
        ...
        if (result) {
            ...
            initializeRemotingServer();  // 初始化 nettyServer

            initializeResources();  // 初始化一系列的线程池

            registerProcessor();   // 注册一系列请求和对应处理

            initializeScheduledTasks();  // 初始化一系列定时任务（包括一些json文件的定时实例化到磁盘）

            initialTransaction();  // 初始化事务相关
    ...
    protected void startBasicService() throws Exception {
        ...
        if (this.messageStore != null) {
            this.messageStore.start();  // 本地日志管理
        }
        ...
        if (this.replicasManager != null) {
            this.replicasManager.start();  // 同步管理
        }
        ...
        if (this.remotingServer != null) {
            this.remotingServer.start();  // 启动服务端监听端口
        ...
        if (this.topicRouteInfoManager != null) {
            this.topicRouteInfoManager.start();  // 连接namesrv获取topic
        }
    ...
    public void start() throws Exception {
        ...
        startBasicService();  // 启动服务端

        if (!isIsolated && !this.messageStoreConfig.isEnableDLegerCommitLog() && !this.messageStoreConfig.isDuplicationEnable()) {
            changeSpecialServiceStatus(this.brokerConfig.getBrokerId() == MixAll.MASTER_ID);  // 启动定时任务，用于检查并处理状态相关的
            this.registerBrokerAll(true, false, true);
        }

        scheduledFutures.add(this.scheduledExecutorService.scheduleAtFixedRate(new AbstractBrokerRunnable(this.getBrokerIdentity()) {
            @Override
            public void run0() {  // 10秒间隔，从nameSrv注册
                try {
    ...
    public void changeSpecialServiceStatus(boolean shouldStart) {
        ...
        changeScheduleServiceStatus(shouldStart);  // 定时改状态的线程，如刷盘、延时消息投递

        changeTransactionCheckServiceStatus(shouldStart);  // 事务检查定时器
    ...
    private synchronized void changeTransactionCheckServiceStatus(boolean shouldStart) {
        if (isTransactionCheckServiceStart != shouldStart) {
            ...
            if (shouldStart) {
                this.transactionalMessageCheckService.start();  // 启动事务检查线程
            } else {
