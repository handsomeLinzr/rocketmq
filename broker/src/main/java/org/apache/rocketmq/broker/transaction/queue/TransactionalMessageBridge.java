...
public class TransactionalMessageBridge {
    ...
    public long fetchConsumeOffset(MessageQueue mq) {
        long offset = brokerController.getConsumerOffsetManager().queryOffset(TransactionalMessageUtil.buildConsumerGroup(),  // 半事务处理组 CID_RMQ_SYS_TRANS
            mq.getTopic(), mq.getQueueId());
        if (offset == -1) {
            offset = store.getMinOffsetInQueue(mq.getTopic(), mq.getQueueId());  // 首次是0
        }
        return offset;
    }
    ...
    public Set<MessageQueue> fetchMessageQueues(String topic) {
        ...
        TopicConfig topicConfig = selectTopicConfig(topic);  // topic
        if (topicConfig != null && topicConfig.getReadQueueNums() > 0) {
            for (int i = 0; i < topicConfig.getReadQueueNums(); i++) {  // readQueue数量
    ...
    public PullResult getOpMessage(int queueId, long offset, int nums) {
        ...
        SubscriptionData sub = new SubscriptionData(topic, "*");
        return getMessage(group, topic, queueId, offset, nums, sub);  // 获取操作队列的数据
    }

    private PullResult getMessage(String group, String topic, int queueId, long offset, int nums,
        SubscriptionData sub) {
        GetMessageResult getMessageResult = store.getMessage(group, topic, queueId, offset, nums, null);  // 从commitLog获取数据
    ...
    private MessageExtBrokerInner parseHalfMessageInner(MessageExtBrokerInner msgInner) {  // 转换半消息
        String uniqId = msgInner.getUserProperty(MessageConst.PROPERTY_UNIQ_CLIENT_MESSAGE_ID_KEYIDX);  // 事务id
        if (uniqId != null && !uniqId.isEmpty()) {
            MessageAccessor.putProperty(msgInner, TransactionalMessageUtil.TRANSACTION_ID, uniqId);
        }
        MessageAccessor.putProperty(msgInner, MessageConst.PROPERTY_REAL_TOPIC, msgInner.getTopic());  // 设置原来的topic
        MessageAccessor.putProperty(msgInner, MessageConst.PROPERTY_REAL_QUEUE_ID,  // 原来的queueId
            String.valueOf(msgInner.getQueueId()));
        msgInner.setSysFlag(
            MessageSysFlag.resetTransactionValue(msgInner.getSysFlag(), MessageSysFlag.TRANSACTION_NOT_TYPE));  // 抹去事务的标识
        msgInner.setTopic(TransactionalMessageUtil.buildHalfTopic());  // RMQ_SYS_TRANS_HALF_TOPIC 半消息
        msgInner.setQueueId(0);  // 0队列
    ...
    public boolean writeOp(Integer queueId,Message message) {
        ...
        PutMessageResult result = putMessageReturnResult(makeOpMessageInner(message, opQueue));  // opMassage写入commitLog
        if (result != null && result.getPutMessageStatus() == PutMessageStatus.PUT_OK) {
            return true;
        }
