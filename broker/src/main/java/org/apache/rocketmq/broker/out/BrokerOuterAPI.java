...
public class BrokerOuterAPI {
    ...
    private BrokerOuterAPI(final NettyClientConfig nettyClientConfig, RPCHook rpcHook, ClientMetadata clientMetadata) {
        ...
        this.remotingClient.registerRPCHook(rpcHook);  // DynamicalExtFieldRPCHook
        this.rpcClient = new RpcClientImpl(this.clientMetadata, this.remotingClient);
    }

    public void start() {
        this.remotingClient.start();  // 启动client
    }
    ...
    public List<RegisterBrokerResult> registerBrokerAll(
        ...
        if (nameServerAddressList != null && nameServerAddressList.size() > 0) {
            ...
            final CountDownLatch countDownLatch = new CountDownLatch(nameServerAddressList.size());  // nameSrv数量
            for (final String namesrvAddr : nameServerAddressList) {  // 每个都发送一次请求注册
