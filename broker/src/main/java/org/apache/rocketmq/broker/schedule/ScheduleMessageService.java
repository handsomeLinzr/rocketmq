...
public class ScheduleMessageService extends ConfigManager {
    ...
    private final ConcurrentMap<Integer /* level */, Long/* delay timeMillis */> delayLevelTable =
        new ConcurrentHashMap<>(32);  // 延迟等级 : 毫秒
    ...
    public void start() {
        if (started.compareAndSet(false, true)) {  // 设置启动标识
            this.load();  // 加载延迟队列相关
            this.deliverExecutorService = new ScheduledThreadPoolExecutor(this.maxDelayLevel, new ThreadFactoryImpl("ScheduleMessageTimerThread_")); // 投递队列，线程数为队列的数量(18)
            ...
            for (Map.Entry<Integer, Long> entry : this.delayLevelTable.entrySet()) {
                ...
                if (timeDelay != null) {
                    ...
                    this.deliverExecutorService.schedule(new DeliverDelayedMessageTimerTask(level, offset), FIRST_DELAY_TIME, TimeUnit.MILLISECONDS);  // 10秒后开始
                }
            }
            ...
            this.deliverExecutorService.scheduleAtFixedRate(new Runnable() {
                ...
                public void run() {
                    try {
                        if (started.get()) {
                            ScheduleMessageService.this.persist();  // 10秒刷盘
                        }
                    } catch (Throwable e) {
                        log.error("scheduleAtFixedRate flush exception", e);
                    }
    ...
    private MessageExtBrokerInner messageTimeup(MessageExt msgExt) {  // msg数据重新写回
    ...
    class DeliverDelayedMessageTimerTask implements Runnable {
        ...
        public void executeOnTimeup() {
            ...
            if (cq == null) {
                this.scheduleNextTimerTask(this.offset, DELAY_FOR_A_WHILE);  // 100 ms 后重新扫描
                return;
            }
            ...
            if (bufferCQ == null) {
                ...
                this.scheduleNextTimerTask(resetOffset, DELAY_FOR_A_WHILE);   // 100ms后重新扫描
                return;
            }
            ...
            try {
                while (bufferCQ.hasNext() && isStarted()) {
                    ...
                    long offsetPy = cqUnit.getPos();  // 偏移
                    int sizePy = cqUnit.getSize();  // 大小
                    long tagsCode = cqUnit.getTagsCode();  // tagsHashcode
                    ...
                    if (!cqUnit.isTagsCodeValid()) {
                        ...
                        tagsCode = computeDeliverTimestamp(delayLevel, msgStoreTime);  // 应该执行的时间
                    }
                    ...
                    assert cqUnit.getBatchNum() == 1;  // 单条继续消费
                    nextOffset = currOffset + cqUnit.getBatchNum();  // 下个offset的脚标偏移

                    long countdown = deliverTimestamp - now;
                    if (countdown > 0) {
                        this.scheduleNextTimerTask(currOffset, DELAY_FOR_A_WHILE);  // 500ms后重新尝试
                        ScheduleMessageService.this.updateOffset(this.delayLevel, currOffset);  // 修改 offsetTable，记录当前等级对应的偏移
                        return;
                    }

                    MessageExt msgExt = ScheduleMessageService.this.brokerController.getMessageStore().lookMessageByOffset(offsetPy, sizePy); // 从commitLog获得数据
                    if (msgExt == null) {
                        continue;
                    }

                    MessageExtBrokerInner msgInner = ScheduleMessageService.this.messageTimeup(msgExt);  // 数据重新写回
                    if (TopicValidator.RMQ_SYS_TRANS_HALF_TOPIC.equals(msgInner.getTopic())) { // 延迟队列不支持半事务消息
                    ...
                    if (!deliverSuc) {
                        this.scheduleNextTimerTask(nextOffset, DELAY_FOR_A_WHILE);  // 100ms再试
                        return;
                    }
                }
            } catch (Exception e) {
        ...
        private PutResultProcess deliverMessage(MessageExtBrokerInner msgInner, String msgId, long offset,
            ...
            CompletableFuture<PutMessageResult> future =  // 消息重写回commitLog
    ...
    public class PutResultProcess {
        ...
        public void onSuccess(PutMessageResult result) {  // 统计
