...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx,
        ...
        switch (request.getCode()) {  // 消费失败重试的发送
    ...
    private boolean handleRetryAndDLQ(SendMessageRequestHeader requestHeader, RemotingCommand response,
        ...
        if (null != newTopic && newTopic.startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX)) {  // 重试队列
            String groupName = newTopic.substring(MixAll.RETRY_GROUP_TOPIC_PREFIX.length());   // 组名
            ...
            int reconsumeTimes = requestHeader.getReconsumeTimes() == null ? 0 : requestHeader.getReconsumeTimes(); // 当前重试的次数
            // Using '>' instead of '>=' to compatible with the case that reconsumeTimes here are increased by client.
            if (reconsumeTimes > maxReconsumeTimes) {  // 超过最大重试次数，topic queue 都换成 DLQ 相关
                ...
                newTopic = MixAll.getDLQTopic(groupName);  // topic 换成 DLQ队列
    ...
    public RemotingCommand sendMessage(final ChannelHandlerContext ctx,
        ...
        final RemotingCommand response = preSend(ctx, request, requestHeader);  // 检查
        ...
        TopicConfig topicConfig = this.brokerController.getTopicConfigManager().selectTopicConfig(requestHeader.getTopic());  // 获取topic配置

        if (queueIdInt < 0) {   // queueId < 0 则随机指定一个writeQueue
        ...
        if (!handleRetryAndDLQ(requestHeader, response, request, msgInner, topicConfig, oriProps)) {  // 处理重试队列的情况
            return response;
        }
        ...
        msgInner.setTagsCode(MessageExtBrokerInner.tagsString2tagsCode(topicConfig.getTopicFilterType(), msgInner.getTags()));  // tags的hashCode
