...
public abstract class AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    protected RemotingCommand consumerSendMsgBack(final ChannelHandlerContext ctx, final RemotingCommand request)
        ...
        if (subscriptionGroupConfig.getRetryQueueNums() <= 0) {  // 剩余重试此时<=0则不处理
            response.setCode(ResponseCode.SUCCESS);
            response.setRemark(null);
            return response;
        }

        String newTopic = MixAll.getRetryTopic(requestHeader.getGroup());  // 新的topic：%RETRY%'group'
        int queueIdInt = this.random.nextInt(subscriptionGroupConfig.getRetryQueueNums());  // 就一个重试队列
        ...
        // Create retry topic to master broker  创建 topic
        ...
        if (null == retryTopic) {
            MessageAccessor.putProperty(msgExt, MessageConst.PROPERTY_RETRY_TOPIC, msgExt.getTopic());  // 设置实际的topic
        }
        msgExt.setWaitStoreMsgOK(false);  // 不需等待响应

        int delayLevel = requestHeader.getDelayLevel();  // 延迟延迟等级

        int maxReconsumeTimes = subscriptionGroupConfig.getRetryMaxTimes();  // 最大重试次数，默认16
        ...
        if (msgExt.getReconsumeTimes() >= maxReconsumeTimes
            || delayLevel < 0) {  // 重试等级 < 0 || 重试次数 > 最大重试次数，进入死信队列
            ...
            Attributes attributes = BrokerMetricsManager.newAttributesBuilder()
                ...
                .build();  // 设置实际的topic和group
            BrokerMetricsManager.sendToDlqMessages.add(1, attributes);

            isDLQ = true;
            newTopic = MixAll.getDLQTopic(requestHeader.getGroup());  // %DLQ%'group'  死信队列
            queueIdInt = randomQueueId(DLQ_NUMS_PER_GROUP);

            // Create DLQ topic to master broker   创建死信队列主题
            ...
            if (0 == delayLevel) {
                delayLevel = 3 + msgExt.getReconsumeTimes();  // 级别为重试次数+3，这里进行级别递增
            }
        ...
        // Put retry topic to master message store  保存
