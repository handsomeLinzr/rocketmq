...
public class TopicConfigManager extends ConfigManager {
    ...
    public TopicConfigManager(BrokerController brokerController) {  // 初始化并存放一系列的topic
        ...
        }
        {
            if (this.brokerController.getBrokerConfig().isAutoCreateTopicEnable()) {
                ...
                topicConfig.setWriteQueueNums(this.brokerController.getBrokerConfig()
                    .getDefaultTopicQueueNums());
                int perm = PermName.PERM_INHERIT | PermName.PERM_READ | PermName.PERM_WRITE;  // 权限
    ...
    public TopicConfig createTopicInSendMessageBackMethod(
        ...
        try {
            if (this.topicConfigTableLock.tryLock(LOCK_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                try {
                    ...
                    topicConfig = new TopicConfig(topic);
                    topicConfig.setReadQueueNums(clientDefaultTopicQueueNums);  // 读
                    topicConfig.setWriteQueueNums(clientDefaultTopicQueueNums);  // 写
                    topicConfig.setPerm(perm);
                    topicConfig.setTopicSysFlag(topicSysFlag);
                    topicConfig.setOrder(isOrder);
        ...
        if (createNew) {  // 新建的topic需要注册到HA节点上
            this.brokerController.registerBrokerAll(false, true, true);
        }
    ...
    public void updateTopicConfig(final TopicConfig topicConfig) {
        ...
        this.persist(topicConfig.getTopicName(), topicConfig);  // 刷盘到文件
    }
