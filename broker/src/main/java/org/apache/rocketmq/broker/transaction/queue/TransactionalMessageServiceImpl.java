...
public class TransactionalMessageServiceImpl implements TransactionalMessageService {
    ...
    public CompletableFuture<PutMessageResult> asyncPrepareMessage(MessageExtBrokerInner messageInner) {
        return transactionalMessageBridge.asyncPutHalfMessage(messageInner);  // 放置半消息
    }
    ...
    private boolean needDiscard(MessageExt msgExt, int transactionCheckMax) {
        String checkTimes = msgExt.getProperty(MessageConst.PROPERTY_TRANSACTION_CHECK_TIMES);  // 已经检查的次数
        int checkTime = 1;
        if (null != checkTimes) {
        ...
        msgExt.putUserProperty(MessageConst.PROPERTY_TRANSACTION_CHECK_TIMES, String.valueOf(checkTime));  // 写回
        return false;
    }

    private boolean needSkip(MessageExt msgExt) {
        long valueOfCurrentMinusBorn = System.currentTimeMillis() - msgExt.getBornTimestamp();  // 间隔的时间
    ...
    public void check(long transactionTimeout, int transactionCheckMax,
        ...
        try {
            String topic = TopicValidator.RMQ_SYS_TRANS_HALF_TOPIC;  // 半事务topic
            ...
            for (MessageQueue messageQueue : msgQueues) {
                ...
                while (true) {
                    if (System.currentTimeMillis() - startTime > MAX_PROCESS_TIME_LIMIT) {  // 60000
                        log.info("Queue={} process time reach max={}", messageQueue, MAX_PROCESS_TIME_LIMIT);
                        break;
                    }
                    if (removeMap.containsKey(i)) {  // commit/rolled back 的数据会记录在op上，所以会出现在removeMap中
                        ...
                        GetResult getResult = getHalfMsg(messageQueue, i);  // 获取半事务消息，1个
                        ...
                        if (needDiscard(msgExt, transactionCheckMax) || needSkip(msgExt)) {
                            listener.resolveDiscardMsg(msgExt);  // 暂时丢弃（扔到一个特殊的topic）
                            newOffset = i + 1;  // 偏移+1，检查下一个
                            i++;
                            continue;
                        }
                        ...
                        boolean isNeedCheck = opMsg == null && valueOfCurrentMinusBorn > checkImmunityTime
                            ...
                            || valueOfCurrentMinusBorn <= -1;   // 存储时间到当前已经超过事务超时时间

                        if (isNeedCheck) {

                            if (!putBackHalfMsgQueue(msgExt, i)) {  // 将半消息再次存入commitLog中
                                continue;
                            }
                            putInQueueCount++;
                            ...
                            listener.resolveHalfMsg(msgExt);  // 调用生产端回查状态
                        } else {
    ...
    private PullResult fillOpRemoveMap(HashMap<Long, Long> removeMap, MessageQueue opQueue,
        ...
        List<MessageExt> opMsg = pullResult.getMsgFoundList();  // 获取数据
        if (opMsg == null) {
        ...
        for (MessageExt opMessageExt : opMsg) {
            ...
            String queueOffsetBody = new String(opMessageExt.getBody(), TransactionalMessageUtil.CHARSET);  // 获取内容(半事务消息的offset)
            ...
            if (TransactionalMessageUtil.REMOVE_TAG.equals(opMessageExt.getTags())) {
                ...
                for (String offset : offsetArray) {
                    ...
                    removeMap.put(offsetValue, opMessageExt.getQueueOffset());   // op已经记录的数据
                    set.add(offsetValue);
                }
            } else {
    ...
    private MessageQueue getOpQueue(MessageQueue messageQueue) {
        ...
        if (opQueue == null) {
            opQueue = new MessageQueue(TransactionalMessageUtil.buildOpTopic(), messageQueue.getBrokerName(),
                messageQueue.getQueueId());   // RMQ_SYS_TRANS_OP_HALF_TOPIC   每个半事务队列 对应一个 操作失误队列
    ...
    private OperationResult getHalfMessageByOffset(long commitLogOffset) {
        ...
        MessageExt messageExt = this.transactionalMessageBridge.lookMessageByOffset(commitLogOffset);  // 获取数据(半消息)
        if (messageExt != null) {
            response.setPrepareMessage(messageExt);  // 填充
    ...
    public boolean deletePrepareMessage(MessageExt messageExt) {
        ...
        String data = messageExt.getQueueOffset() + TransactionalMessageUtil.OFFSET_SEPARATOR;  // 数据，即使queueOffset+","
        try {
            boolean res = mqContext.getContextQueue().offer(data, 100, TimeUnit.MILLISECONDS);  // 数据存入
            if (res) {
                int totalSize = mqContext.getTotalSize().addAndGet(data.length());  // 统计
                if (totalSize > transactionalMessageBridge.getBrokerController().getBrokerConfig().getTransactionOpMsgMaxSize()) {
                    this.transactionalOpBatchService.wakeup();   // 唤醒等待线程
                }
                return true;
            } else {
        ...
        Message msg = getOpMessage(queueId, data);  // 封装OP队列数据
        if (this.transactionalMessageBridge.writeOp(queueId, msg)) {  // 写入commitLog
    ...
    public Message getOpMessage(int queueId, String moreData) {
        String opTopic = TransactionalMessageUtil.buildOpTopic();  // RMQ_SYS_TRANS_OP_HALF_TOPIC
        MessageQueueOpContext mqContext = deleteContext.get(queueId);
        ...
        return new Message(opTopic, TransactionalMessageUtil.REMOVE_TAG,  // REMOVE_TAG=d
                sb.toString().getBytes(TransactionalMessageUtil.CHARSET));  // 构建成带tag的消息
    }
