...
public class NamesrvStartup {
    ...
    public static NamesrvController main0(String[] args) {
        try {
            parseCommandlineAndConfigFile(args);  // 解析配置
    ...
    public static void parseCommandlineAndConfigFile(String[] args) throws Exception {
        ...
        Options options = ServerUtil.buildCommandlineOptions(new Options());  // 命令行参数提醒和校验
        ...
        if (commandLine.hasOption('c')) {  // 加载配置文件namesrvConfig、nettyServerConfig、nettyClientConfig
            String file = commandLine.getOptionValue('c');
            if (file != null) {
                InputStream in = new BufferedInputStream(Files.newInputStream(Paths.get(file)));
                properties = new Properties();
                properties.load(in);
                MixAll.properties2Object(properties, namesrvConfig);  // 这种设置方式很暴力。。。直接全部set一遍
    ...
    public static NamesrvController createAndStartNamesrvController() throws Exception {
        ...
        NamesrvController controller = createNamesrvController();  // 创建controller
        start(controller);  // 启动
